# 🚀 快速部署到 Render（後端）

## 📋 準備工作

✅ 已完成：
- `server.js` 已更新，包含所有 API 端點
- `render.yaml` 配置文件已準備
- 所有代碼已準備就緒

## 🎯 部署步驟（約 5 分鐘）

### 步驟 1：在 Render 建立 Web Service

1. **前往 Render Dashboard**
   - 開啟 https://dashboard.render.com/
   - 確認您已登入

2. **建立新的 Web Service**
   - 點擊右上角 "New +" 按鈕
   - 選擇 "Web Service"
   - 選擇 "Build and deploy from a Git repository"
   - 如果還沒連接 GitHub，點擊 "Connect account" 連接您的 GitHub 帳號
   - 選擇 `ai-pastor` 倉庫（或您的倉庫名稱）

3. **設定服務配置**
   - **Name**: `ai-pastor-backend`（或您喜歡的名稱）
   - **Region**: 選擇離您最近的區域（建議選擇與 Vercel 相同的區域，通常是 `Oregon (US West)` 或 `Singapore`）
   - **Branch**: `main`（或您的主要分支）
   - **Root Directory**: 留空（使用根目錄）
   - **Runtime**: `Node`
   - **Build Command**: `npm install`
   - **Start Command**: `node server.js`
   - **Plan**: 選擇 `Free`（或付費方案）

4. **設定環境變數**
   在 "Environment Variables" 區塊中：
   - 點擊 "Add Environment Variable"
   - **Key**: `GOOGLE_API_KEY`
   - **Value**: 您的 Google Gemini API Key
   - 點擊 "Add"
   - （可選）添加 `NODE_ENV` = `production`

5. **建立服務**
   - 點擊 "Create Web Service"
   - Render 會自動開始建置和部署
   - 等待部署完成（約 2-3 分鐘）

### 步驟 2：獲得後端 URL

部署完成後：
- 在服務頁面頂部會顯示 URL
- 格式類似：`https://ai-pastor-backend.onrender.com`
- **複製這個 URL！**

### 步驟 3：更新前端配置

獲得 Render 後端 URL 後，更新 `index.html`：

1. **編輯 `index.html` 第 325 行**
   
   找到：
   ```javascript
   const API_BASE_URL = window.API_BASE_URL || '';
   ```
   
   改為（替換為您的 Render URL）：
   ```javascript
   const API_BASE_URL = window.API_BASE_URL || 'https://ai-pastor-backend.onrender.com';
   ```

2. **提交並推送更改**
   ```bash
   git add index.html
   git commit -m "Update API_BASE_URL to point to Render backend"
   git push origin main
   ```

3. **Vercel 自動重新部署**
   - Vercel 會自動偵測到 GitHub 的更改
   - 自動觸發新的部署
   - 在 Vercel Dashboard 可以看到新部署

### 步驟 4：驗證部署

1. **測試後端**
   - 在瀏覽器開啟您的 Render 後端 URL
   - 應該會看到一些回應（可能是錯誤頁面，這是正常的）
   - 測試健康檢查：`https://your-backend-url.onrender.com/health`
   - 應該返回：`{"status":"ok","timestamp":"..."}`

2. **測試前端**
   - 開啟您的 Vercel 前端：`https://ai-pastor.vercel.app`
   - 按 `F12` 開啟開發者工具
   - 切換到 "Network" 標籤
   - 在 AI 牧師中發送一條測試訊息
   - 檢查 Network 標籤：
     - ✅ 應該看到請求發送到您的 Render 後端 URL（`/api/chat`）
     - ✅ 應該收到正常的回應
     - ❌ **不應該**看到直接呼叫 `generativelanguage.googleapis.com` 的請求

3. **測試認證功能**
   - 嘗試註冊一個新帳號
   - 檢查是否能成功註冊和登入
   - 檢查對話記錄是否能正常保存

## ⚠️ 重要注意事項

### Render 免費方案限制

- **服務休眠**：如果 15 分鐘內沒有請求，免費服務會進入休眠狀態
- **首次請求延遲**：休眠後首次請求可能需要 30-60 秒來喚醒服務
- **解決方案**：
  - 使用付費方案（$7/月起）可避免休眠
  - 或設定定期 ping 服務以保持活躍（可以使用 cron job 或外部服務）

### 健康檢查

Render 會自動檢查服務是否正常運行。`server.js` 中的 `/health` 端點可以用作健康檢查。

## 🎉 完成！

完成後，您將擁有：
- ✅ 前端：`https://ai-pastor.vercel.app`（Vercel）
- ✅ 後端：`https://ai-pastor-backend.onrender.com`（Render）
- ✅ 所有功能正常運作

## 📞 遇到問題？

1. **檢查 Render Logs**
   - 在 Render Dashboard 中查看 "Logs" 標籤
   - 查看是否有錯誤訊息

2. **檢查環境變數**
   - 確認 `GOOGLE_API_KEY` 是否正確設定
   - 確認 API Key 是否有效

3. **檢查前端配置**
   - 確認 `API_BASE_URL` 是否正確指向 Render 後端
   - 檢查瀏覽器 Console 是否有 CORS 錯誤

4. **測試 API 端點**
   - 使用 Postman 或 curl 測試後端 API
   - 確認後端是否正常回應




